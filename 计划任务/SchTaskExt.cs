    ...
    public class SchTaskExt
    {
        ...
        public static _TASK_STATE CreateTaskScheduler(string creator, string taskName, string path, string interval, string StartBoundary)
        {
            try
            {
                ...
                task.RegistrationInfo.Author = "YUHAO TANG";//creator
                task.RegistrationInfo.Description = "Runs every Sunday.";//description
                task.Principal.RunLevel = _TASK_RUNLEVEL.TASK_RUNLEVEL_HIGHEST; //使用最高权限运
                ...
                //tt.StartBoundary = DateTime.Now.ToString("yyyy-MM-dd") + "T" + DateTime.Now.ToString("T");//"2015-04-09T14:27:25";//start time
                tt.StartBoundary = StartBoundary;
                //set action
                ...
                IRegisteredTask regTask = folder.RegisterTaskDefinition(taskName, task,
                                                                    ...
                                                                    "");
                IRunningTask runTask = regTask.Run(null);//.Run(null);
